function [xc, yc, sigma] = radialcenter(I,dIdu,dIdv,fdu,fdv)
...
% Optional Inputs:
%   dIdu,dIdv,fdv,fdu
%   Precalculate the image derivative:
%       dIdu = I(1:end-1,2:end)-I(2:end,1:end-1);
%       dIdv = I(1:end-1,1:end-1)-I(2:end,2:end);
%   and then calculate a smoothed version:
%       h = ones(3)/9;  % simple 3x3 averaging filter
%       fdv = conv2(dIdv, h, 'same');
%       fdu = conv2(dIdu, h, 'same');
%   pass the region of interest for each into the function.  Arrays should
%   be the one pixel shorter and narrower than I, but starting at the same
%   initial position.
%%
% Outputs
...
%%
% Change Log:
% 2014-04-15 Dan Kovari, Georgia Tech School of Physics
%       Tweaked multiple statements throughout the algorithm to increase
%       the speed. Added the option to precalculate the image derivative
%       and smoothed derivative, to decrease the massive overhead that
%       mutliple calls to conv2 caused.
%%
...
if nargin < 5   %give the option to precalculate the image derivative and smoothing function for speed increase
...
if any(isnan(m(:)))
    ...
    m(isnan(m))=(dIdv(isnan(m)) + dIdu(isnan(m))) ./ (dIdu(isnan(m))-dIdv(isnan(m)));%unsmoothm(isnan(m));
end
...
try
    ...
    m(isinf(m)) = .9*realmax;   %try to replace inf with a really large value
catch
...
% A rough measure of the particle width. Adds some time to the algorithm so
% only calculate if user asks for it.
if nargout>2
    Isub = I - min(I(:));

    [rx2,ry2] = meshgrid( ((1:Nx)-xc).^2, ((1:Ny)-yc).^2);
    r2 = rx2+ry2;
